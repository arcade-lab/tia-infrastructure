...
int main(int argc, char **argv)
{
    ...
    size_t num_individual_program_binary_words, expected_num_program_binary_words, num_program_binary_words,
           num_scratchpad_data_words, num_input_data_words, num_expected_output_data_words, i;
    bool has_scratchpad_data, passed;
    int dev_uio0_fd, ret;
    volatile uint32_t *base_address, *system_control_base_address, *block_base_address,
                      *data_memory_base_address;
    system_control_t system_control;
    block_t block;
    processing_element_t *processing_element;
    const char *labels[16] = {"pe_0",
                              "pe_1",
                              "pe_2",
                              "pe_3",
                              "pe_4",
                              ...
                              "pe_14",
                              "pe_15"};
    cJSON *results, *processing_element_results[16];

    // --- Argument Parsing and File I/O ---

    // Check usage.
    if (argc != 6) {
        fprintf(stderr, "Usage: ./qte PLATFORM_FILE PROGRAM_BINARY SCRATCHPAD_DATA INPUT_DATA EXPECTED_OUTPUT_DATA\n");
        return 1;
    }
    ...
    num_individual_program_binary_words = platform->core->num_registers
                                          + platform->core->num_instructions
                                            * platform->core->mm_instruction_width
                                            / platform->system->host_word_width;
    expected_num_program_binary_words = 4 * num_individual_program_binary_words;
    ...
    // Processing element.
    block_base_address = &system_control_base_address[system_control.num_system_control_register_words];
    initialize_block(&block, block_base_address, platform);

    // Data memory.
    data_memory_base_address = &block_base_address[block.num_address_space_words];
    ...
    // Program the device.
    for (i = 0; i < 16; i++) {
        processing_element = get_logical_processing_element(&block, i);
        ret = write_processing_element_program(processing_element,
                                               &program_binary[i * num_individual_program_binary_words]);
        if (ret < 0) {
            fprintf(stderr, "Failed to write the program to the processing element %zu instruction memory.\n", i);
    ...
    // TODO: settle on multi-PE scratchpad memory format.
    // Write scratchpad data, if needed.
    if (has_scratchpad_data) {
        ret = write_to_processing_element_scratchpad(&block.quartets[0].processing_elements[0], scratchpad_data);
        if (ret < 0) {
    ...
    for (i = 0; i < 16; i++) {
        processing_element_results[i] = create_results_object();
        add_header(results, labels[i], processing_element_results[i]);
        processing_element = get_logical_processing_element(&block, i);
        build_processing_element_results(processing_element, processing_element_results[i]);
        fprintf(stderr, "\n--- Processing Element %zu ---\n\n", i);
        display_debug_info(processing_element);
        fprintf(stderr, "\n");
    }
