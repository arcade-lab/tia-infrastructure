...
int main(int argc, char **argv)
{
    ...
    int dev_uio0_fd, ret;
    volatile uint32_t *base_address, *system_control_base_address, *quartet_base_address,
                      *data_memory_base_address;
    system_control_t system_control;
    quartet_t quartet;
    const char *labels[4] = {"pe_0", "pe_1", "pe_2", "pe_3"};
    cJSON *results, *processing_element_results[4];
    ...
    // Processing element.
    quartet_base_address = &system_control_base_address[system_control.num_system_control_register_words];
    initialize_quartet(&quartet, quartet_base_address, platform);

    // Data memory.
    data_memory_base_address = &quartet_base_address[quartet.num_address_space_words];
    ...
    // Program the device.
    for (i = 0; i < 4; i++) {
        ret = write_processing_element_program(&quartet.processing_elements[i],
    ...
    if (has_scratchpad_data) {
        ret = write_to_processing_element_scratchpad(&quartet.processing_elements[0], scratchpad_data);
        if (ret < 0) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        build_processing_element_results(&quartet.processing_elements[i], processing_element_results[i]);
        fprintf(stderr, "\n--- Processing Element %zu ---\n\n", i);
        display_debug_info(&quartet.processing_elements[i]);
        fprintf(stderr, "\n");
    }
